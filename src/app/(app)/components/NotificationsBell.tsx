...
import { FiBell, FiCheckCircle, FiAlertCircle, FiBarChart2 } from "react-icons/fi";
...
function getIconForNotification(n: Notification) {
  ...
  switch (n.type) {
    case "post_published":
      return (
        <div className={`${baseClass} bg-emerald-500/10 border border-emerald-500/40 text-emerald-300`}>
          <FiCheckCircle size={14} />
        </div>
      );
    case "post_publish_failed":
      return (
        <div className={`${baseClass} bg-red-500/10 border border-red-500/40 text-red-300`}>
    ...
    default:
      return (
        <div className={`${baseClass} bg-sky-500/10 border border-sky-500/40 text-sky-300`}>
          <FiBarChart2 size={14} />
        </div>
      );
  }
...
export function NotificationsBell({ workspaceId, uid }: NotificationsBellProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          className="absolute right-0 mt-2 w-80 rounded-2xl border border-[#261341] bg-[#050017] shadow-2xl z-50 overflow-hidden"
        >
              ...
              return (
                <button
                  ...
                  onClick={async () => {
                    ...
                    // aqui no futuro podemos navegar para uma página de detalhes do post
                    // ex.: router.push(`/posts/${n.postId}`)
                  }}
